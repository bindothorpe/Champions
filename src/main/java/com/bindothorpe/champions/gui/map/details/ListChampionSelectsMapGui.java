...
public class ListChampionSelectsMapGui extends PlayerGui {
    private static final int WIDTH = 9;
    private static final int HEIGHT = 4;
    public static final int MAX_CHAMPION_SELECTS = 14;

    private final GameMap gameMap;

    private StaticPane rootPane;

    public ListChampionSelectsMapGui(UUID uuid, DomainController dc, GameMap gameMap) {
        super(uuid, dc);
    ...
    protected void initialize() {
        gui = new ChestGui(HEIGHT, "Champion Selects");
        ...
        rootPane.addItem(new MenuChampionSelectsItem(dc, true, gameMap, uuid -> new ListChampionSelectsMapGui(uuid, dc, gameMap).open()), Slot.fromXY(4, 0));
        ...
        gameMap.getGameObjects().stream()
                .filter((gameObject -> gameObject instanceof ChampionSelectGameObject))
                .limit(MAX_CHAMPION_SELECTS)
                .forEach(gameObject -> rootPane.addItem(new ChampionSelectInstanceItem(dc, (ChampionSelectGameObject) gameObject, gameMap), counter.get() % 7, counter.getAndIncrement() / 7 + 1));
    }
